...
# 4. KIỂM TRA KẾT NỐI MẠNG & BACKEND API
Write-Host "4. KIỂM TRA KẾT NỐI MẠNG & BACKEND API:" -ForegroundColor White
if (Test-NetConnection -ComputerName 8.8.8.8 -WarningAction SilentlyContinue | Select-Object -ExpandProperty PingSucceeded) {
    Write-Host "   [OK] Ket noi den Internet: THANH CONG" -ForegroundColor Green
} else {
    Write-Host "   [FAIL] Ket noi den Internet: THAT BAI" -ForegroundColor Red
}

try {
    $backendTest = Invoke-WebRequest -Uri "http://127.0.0.1:8000/api/health" -Method Get -TimeoutSec 5 -ErrorAction Stop
    Write-Host "   [OK] Ket noi den Backend API: THANH CONG" -ForegroundColor Green
} catch {
    Write-Host "   [FAIL] Ket noi den Backend API: THAT BAI (Hoac API dang offline)" -ForegroundColor Red
}    
$passCount++
