...
5. **Mật khẩu chủ mở khóa Agent:** giá trị của `SYSTEM_ADMIN_PASSWORD` là Master Password dùng để giải khóa màn hình máy con khẩn cấp tại `POST /api/auth/verify-password`. Vì endpoint này nằm trên mạng công khai, mật khẩu chủ **phải là bí mật thật** (đặt trên Vercel, không ghi vào code/docs). Khi biến môi trường rỗng thì endpoint này chỉ chấp nhận mật khẩu của tài khoản admin trong DB.
