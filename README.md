...
- backend_api/.env (được docker-compose sử dụng)
  - API_KEY=<API_KEY_của_bạn>                            # Tạo key ngẫu nhiên, xem .env.example
...
- manager-web/.env (dùng khi chạy Vite hoặc build)
  ...
  - VITE_API_KEY=<API_KEY_của_bạn>                       # Phải khớp với backend_api/.env

- agent/.env (agent chạy trên máy đích)
  - API_BASE_URL=http://localhost:8000/api
  - API_KEY=<API_KEY_của_bạn>                            # Phải khớp với backend_api/.env
  - DEVICE_NAME, AGENT_PASSWORD, SEND_INTERVAL (các biến khác theo .env.example)
...
# tạo .env từ .env.example và chỉnh sửa các giá trị: API_BASE_URL, API_KEY, DEVICE_NAME, AGENT_PASSWORD
...
3. Nếu gặp lỗi 401: kiểm tra `agent/.env` API_KEY khớp `backend_api/.env` API_KEY; kiểm tra `agent/communication/` và `manager-web/src/supabase.js` gửi header `Authorization: Bearer <key>`.
...
- Agent 401 Unauthorized: kiểm tra API_KEY và header Bearer trên cả agent và frontend.
...
- backend_api/
  - main.py, core/, routers/, .env, storage/
- manager-web/
  - src/, src/lib/api.js, .env, Dockerfile
- agent/
  - main.py, protection/watchdog.py, communication/, enforcement/, local_store/, requirements.txt, .env.example
- docker-compose.yml
...
│   ├── main.py                    # Bộ não điều phối chính của Agent
│   ├── protection/                # Các module giám sát bảo vệ Agent
│   │   └── watchdog.py            # Tiến trình Giám sát & Tự động Rollback
│   ├── enforcement/               # Các module thực thi lệnh (Blocker, App Rules)
│   ├── communication/             # Nhận và gửi dữ liệu lên Backend API
│   ├── local_store/               # SQLite Local Database & Sync
│   └── utils/                     # Tiện ích mở rộng
│
├── backend_api/                   # Backend API (FastAPI)
│   ├── core/                      # Core configs & security
│   ├── routers/                   # REST API Endpoints
│   ├── storage/                   # Chứa ảnh chụp màn hình cục bộ
...
## 📜 Giấy Phép & Bảo Mật
Dự án được thiết kế chuyên biệt cho hệ thống quản lý thiết bị gia đình (Parental Control System). Toàn bộ dữ liệu được bảo mật bằng API Key và cấu trúc SQLite cục bộ.
