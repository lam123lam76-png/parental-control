    ...
    <div className={`p-4 sm:p-5 rounded-xl border space-y-4 ${styles.card}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className={`p-3 rounded-lg border flex flex-col gap-1 ${styles.card}`}>
          <span className={`text-[10px] font-bold uppercase ${styles.textMuted}`}>Tổng Hoạt Động Tuần</span>
          <div className="flex items-baseline gap-2">
            <span className={`text-xl font-extrabold ${styles.textBold}`}>{analytics.total_logs_week}</span>
            <span className="text-[10px] opacity-70">phiên ghi</span>
          </div>
        </div>
          ...
          <div className="flex items-center gap-1.5">
            {isTrendUp ? (
              <>
                ...
                <span className="text-sm font-bold text-rose-400">+{analytics.trend_percentage}%</span>
                <span className="text-[10px] text-rose-300/70">(Dùng nhiều hơn)</span>
              </>
            ) : (
              <>
                <TrendingDown className="w-4 h-4 text-emerald-400" />
                <span className="text-sm font-bold text-emerald-400">{analytics.trend_percentage}%</span>
                <span className="text-[10px] text-emerald-300/70">(Giảm thời gian)</span>
              </>
            )}
          </div>
        </div>
        ...
        <div className={`p-3 rounded-lg border flex flex-col gap-1 ${styles.card}`}>
          ...
          <span className="text-xs font-bold text-cyan-300">
            {analytics.total_logs_week > 1000 ? "⚠️ Cần chú ý thời lượng" : "🟢 Mức độ sử dụng điều độ"}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-2 pt-2 border-t border-zinc-800/60">
        <span className={`text-[11px] font-bold ${styles.textBold}`}>Phân bố thời lượng theo ngày (Thứ 2 - CN)</span>
        <div className="grid grid-cols-7 gap-1.5 pt-2">
          {analytics.daily_trend?.map((item, idx) => {
            const heightPct = Math.max(8, Math.round((item.count / maxDaily) * 100));
            return (
              <div key={idx} className="flex flex-col items-center gap-1.5">
                <span className="text-[9px] font-mono opacity-70">{item.count}</span>
                <div className="w-full h-16 bg-zinc-800/80 rounded-md flex items-end p-1 overflow-hidden">
                  <div
                    className="w-full bg-gradient-to-t from-cyan-600 to-emerald-400 rounded transition-all duration-500"
                    style={{ height: `${heightPct}%` }}
                  />
                </div>
                <span className="text-[9px] font-bold truncate max-w-full">{item.day}</span>
              </div>
            );
          })}
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-2 border-t border-zinc-800/60">
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-1.5 text-[11px] font-bold text-amber-400">
            ...
            <span>Top 5 Ứng Dụng Mở Nhiều Nhất</span>
          </div>
              ...
              analytics.top_apps?.map((app, idx) => (
                <div key={idx} className="space-y-0.5">
                  <div className="flex items-center justify-between text-xs font-mono">
                    ...
                    <span className="text-[10px] opacity-70">{app.count} lần</span>
                  </div>
          ...
          <div className="space-y-1.5">
            {analytics.top_sites?.length === 0 ? (
              ...
              analytics.top_sites?.map((site, idx) => (
                <div key={idx} className="p-1.5 rounded bg-zinc-900/60 border border-zinc-800/60 flex items-center justify-between text-xs">
                  <span className="font-mono text-cyan-300 truncate max-w-[170px]">{site.domain}</span>
