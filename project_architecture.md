...
Hệ thống được thiết kế theo mô hình **Agent-Server-Client (Thực thi ngầm - Supabase Cloud - Web App quản lý)**:

```mermaid
graph TD
    A[Python Agent trên máy em] <-->|Supabase API & Realtime| B[(Supabase Cloud Database & Storage)]
    C[Web App Quản lý - React/Vite] <-->|Supabase API| B
    A -->|Gửi cảnh báo/Nhật ký| D[Telegram Bot]
    C -->|Vercel Deploy| E[Trình duyệt Admin / Điện thoại]
```
...
1. **Lắng nghe lệnh tức thì (`process_pending_commands`):**
   ...
   - Nếu có lệnh `force_update` -> Tự động nạp bản cập nhật mới từ Supabase Storage, giải nén đè và khởi động lại.
...
7. **Chụp màn hình Định kỳ (`take_screenshot`):**
   - Chụp toàn bộ màn hình, vẽ Timestamp (Giờ:Phút:Giây) màu đỏ lên ảnh, upload lên Supabase Storage bucket `screenshots`.
...
- Tích hợp 2 cơ chế mở khóa:
  ...
  - **Mở khóa từ xa:** Mỗi 10s tự kiểm tra Supabase, nếu Admin bật Tạm dừng/Cho phép mở máy -> Tự đóng màn hình khóa.
- Tích hợp nút **Tắt máy (Shutdown)** để em trai có thể tắt máy nếu đã hết giờ.

---

### Process 3: Quy trình Cập Nhật Từ Xa (Remote Auto Update)
1. Bạn sửa code Agent trên máy bạn.
2. Chạy file `pack_update.bat`:
   - Nén thư mục `agent` -> `update_ver/agent_update.zip`.
   - Tự động gọi `push_update.py` đẩy file nén lên Supabase Storage `agent-updates`.
   - Cập nhật bảng `agent_versions` (`is_latest = True`).
   - Tạo lệnh `force_update` trong bảng `system_commands`.
3. Agent trên máy em trai nhận lệnh `force_update` -> Tải file zip mới nhất -> Giải nén đè lên `C:\ProgramData\ParentalControl` -> Khởi động lại phiên bản mới.
...
| **Màn hình khóa** | `agent/monitor/blocker.py` | Giao diện màn hình khóa, số lượng nút bấm, logic mở khóa/tắt máy. |
| **Giới hạn giờ** | `agent/monitor/time_checker.py` | Thuật toán đếm giờ theo khung giờ hoặc tổng giờ tối đa/ngày. |
| **Quy tắc App** | `agent/monitor/app_rules.py` | Danh sách app cấm, thuật toán diệt tiến trình. |
| **Quy tắc Web** | `agent/monitor/web_rules.py` | Danh sách web cấm, thuật toán quét URL trình duyệt. |
| **Chụp ảnh** | `agent/monitor/screenshot.py` | Độ phân giải ảnh, viền chữ timestamp, định dạng lưu. |
| **Web App UI** | `manager-web/src/App.jsx` | Giao diện quản lý, các bảng thống kê, nút bấm điều khiển, layout. |
| **Script Đóng gói** | `pack_update.bat` / `push_update.py` | Logic tự động nén & đẩy code cập nhật lên cloud. |
